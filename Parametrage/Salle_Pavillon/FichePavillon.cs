...
{
    public partial class FichePavillon : Form
    {
        Model mdl = new Model();
        public FichePavillon()
        {
            InitializeComponent();
        }

        private void FichePavillon_Load(object sender, EventArgs e)
        {
            mdl.Bind("select * from service", paramDataSet.service);
            mdl.Bind(dataGridView1, bindingNavigator1, paramDataSet.Pavillon, pavillonBindingSource, "select * from pavillon");
